#!/usr/bin/env bash
# =============================================================================
# Pi0.5 finetune on Wuji 毛笔 writing bundle (action_dim=54), 2 node x 8 GPU.
# Sibling of train_pi05_writing_8gpu.sh -- doubles the box count to match VAM's
# 16-GPU writing baseline schedule. Both per-device batch and per-step learning
# rate stay the same as the 8-GPU launcher (bs=32 / 16 GPU = 2 per device).
# =============================================================================
set -euo pipefail
...
# global_bs = 16 GPU x per_device 2 = 32. Per-device matches the 8-GPU sibling
# so step semantics (lr/grad-norm/etc) are unchanged -- a 16-GPU run sees double
# the effective dataset throughput per step.
...
NORM_STATS_DIR=${WORK_DIR}/assets/pi05_writing/writing-bundle-teleop
...
RUN_TAG="${RUN_TAG:-pi05_writing_16g}"
...
# === Preflight ==============================================================
echo "=== pi05_writing 16G launch preflight ==="
...
echo "  ckpt_out         = ${CHECKPOINT_BASE_DIR}/pi05_writing/${EXP_NAME}"
...
test -f "${NORM_STATS_DIR}/norm_stats.json" || {
    ...
    echo "       Run: ${VENV_PYTHON} ${WORK_DIR}/scripts/compute_norm_stats.py --config-name=pi05_writing --max-frames 10000" >&2
    exit 1
}
