#!/usr/bin/env bash
# =============================================================================
# Pi0.5 finetune on Wuji 毛笔 writing bundle (action_dim=54) -- baseline for
# ReCamMaster wam-baseline comparison. Sibling of
# train_pi05_robotwin_icl_arx5_8gpu.sh; differs in dataset, norm-stats asset,
# base ckpt (action_dim=54-preprocessed via scripts/convert_base_ckpt_action_dim.py),
# and config name.
#
# Schedule mirrors the RoboTwin pi05 baseline: global batch 16, lr 5e-5
...
# === Preflight ==============================================================
echo "=== pi05_writing 8G launch preflight ==="
...
test -f "${NORM_STATS_DIR}/norm_stats.json" || {
    ...
    echo "       Run: ${VENV_PYTHON} ${WORK_DIR}/scripts/compute_norm_stats.py --config-name=pi05_writing" >&2
    exit 1
}
