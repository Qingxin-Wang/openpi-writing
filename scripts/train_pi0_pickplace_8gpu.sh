#!/usr/bin/env bash
# =============================================================================
# Pi0 finetune on Wuji pick_and_place bundle (action_dim=54) -- sibling of
# train_pi05_pickplace_8gpu.sh; only the base ckpt and config name differ.
# =============================================================================
set -euo pipefail
...
NORM_STATS_DIR=${WORK_DIR}/assets/pi0_pickplace/pickplace-bundle-teleop
...
# === Preflight ==============================================================
echo "=== pi0_pickplace 8G launch preflight ==="
...
echo "  ckpt_out         = ${CHECKPOINT_BASE_DIR}/pi0_pickplace/${EXP_NAME}"
...
test -f "${NORM_STATS_DIR}/norm_stats.json" || {
    ...
    echo "       Run: ${VENV_PYTHON} ${WORK_DIR}/scripts/compute_norm_stats.py --config-name=pi0_pickplace" >&2
    exit 1
}
