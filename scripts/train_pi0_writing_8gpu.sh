#!/usr/bin/env bash
# =============================================================================
# Pi0 finetune on Wuji 毛笔 writing bundle (action_dim=54) -- baseline for
# ReCamMaster wam-baseline comparison. Sibling of train_pi05_writing_8gpu.sh;
# only the base ckpt and config name differ.
# =============================================================================
set -euo pipefail
...
# === Preflight ==============================================================
echo "=== pi0_writing 8G launch preflight ==="
...
test -f "${NORM_STATS_DIR}/norm_stats.json" || {
    ...
    echo "       Run: ${VENV_PYTHON} ${WORK_DIR}/scripts/compute_norm_stats.py --config-name=pi0_writing" >&2
    exit 1
}
