#!/usr/bin/env bash
# =============================================================================
# Pi0.5 finetune on Wuji pnp_left_500 LeRobot bundle (action_dim=54).
#
# Data: /wuji-vepfs/wuji-il/huangsiqiao/data/pnp_left_500_lerobot
#       502 ep / 154,059 frames / 30 fps, single task
#       ("Pick up the object with the left hand and place it in the basket."),
#       54-D dual ARX-5 + dual dex hands, 3 cams (head + L wrist + R wrist).
#
# Schedule (per user request): 100k steps, save+keep every 20k.
# Same a54 base ckpt and NoOpWeightLoader as pi05_pickplace / pi05_writing.
# =============================================================================
set -euo pipefail
...
# === Schedule (env-overridable; defaults match TrainConfig in src) ==========
...
# (20k/40k/60k/80k/100k all preserved).
...
DATASET_DIR=/wuji-vepfs/wuji-il/huangsiqiao/data/pnp_left_500_lerobot
NORM_STATS_DIR=${WORK_DIR}/assets/pi05_pnp_left_500/pnp-left-500-lerobot
...
RUN_TAG="${RUN_TAG:-pi05_pnp_left_500}"
...
# === Preflight ==============================================================
echo "=== pi05_pnp_left_500 8G launch preflight ==="
...
echo "  ckpt_out         = ${CHECKPOINT_BASE_DIR}/pi05_pnp_left_500/${EXP_NAME}"
...
test -f "${NORM_STATS_DIR}/norm_stats.json" || {
    ...
    echo "       Run: ${VENV_PYTHON} ${WORK_DIR}/scripts/compute_norm_stats.py --config-name=pi05_pnp_left_500" >&2
    exit 1
}
